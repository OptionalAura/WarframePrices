/*
 * Copyright 2022 Daniel Allen
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import org.json.JSONObject;

import javax.swing.*;
import java.awt.*;
import java.io.IOException;
import java.util.ArrayDeque;
import java.util.ArrayList;

public class DelayedThreadQueue extends Thread{
    private final ArrayDeque<MarketAPI.Pair<Item, Boolean>> itemQueue;
    private final long delay;
    private volatile boolean shouldRun = true;
    private volatile boolean paused = false;
    private final Application app;

    public DelayedThreadQueue(long delay, final Application application) {
        itemQueue = new ArrayDeque<>();
        this.delay = delay;
        this.app = application;
    }

    public ArrayDeque<MarketAPI.Pair<Item, Boolean>> getQueue() {
        return itemQueue;
    }

    public void addTask(Item r) {
        addTask(r, true);
    }

    public void addTask(Item r, boolean repeat) {
        itemQueue.add(new MarketAPI.Pair<>(r, repeat));
    }

    public void pushTask(String name, int i) {
        Item item = new Item(name, i);
        pushTask(item);
    }

    public void pushTask(Item r) {
        pushTask(r, true);
    }

    public void pushTask(Item r, boolean repeat) {
        itemQueue.push(new MarketAPI.Pair<>(r, repeat));
    }

    @Override
    public void run() {
        while (shouldRun) if (!paused) {
            MarketAPI.Pair<Item, Boolean> itemEntry = itemQueue.pollLast();
            if (itemEntry != null) {
                Item item = itemEntry.left;
                if (item != null) {
                    queueTask(item, itemEntry.right);
                    try {
                        Thread.sleep(delay);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                }
            }
        }
    }

    public void queueTask(Item item, boolean repeat) {
        MarketAPI.Pair<Structure.Order, Structure.Order> order;
        try {
            String name = item.name;
            int loc = item.location;
            boolean avgCached = MarketAPI.isCached(name, "90days");
            JSONObject json = MarketAPI.getObject(name);
            order = MarketAPI.getBestBuyAndSellOrders(json.getJSONObject("payload"));
            if (!item.initialized) {
                item = new Item(json.getJSONObject("include").getJSONObject("item"));
                item.location = loc;
            }

            Thread.sleep(500);
            double avg90d = Math.round(MarketAPI.getAveragePrice90Days(name) * 100) / 100d;
            double avg48h = Math.round(MarketAPI.getAveragePrice48Hours(name) * 100) / 100d;
            ArrayList<Double> orderPrices = MarketAPI.getPrices90Days(name);

            Trends.LinearTrend trend = Trends.getLinearTrend(orderPrices);
            if (!avgCached) Thread.sleep(500);
            //if the item is new or has too few orders, try to get orders from a shorter time span
            if (orderPrices.size() < 60) orderPrices = MarketAPI.getPrices48Hours(name);
            String trendName = "Even";
            if (trend.getDirection() == -1) trendName = "Decreasing";
            else if (trend.getDirection() == 1) trendName = "Increasing";
            Integer profit = null;
            if (order.left != null && order.right != null)
                profit = (int) Math.max(order.left.price - order.right.price, Math.min(avg90d, avg48h) - order.right.price);
            else if (order.right != null) profit = (int) Math.min(avg90d, avg48h) - order.right.price;

            item.buyOrder = order.left;
            item.sellOrder = order.right;
            item.trendValue = trend.a;
            item.trendName = trendName;
            item.profit = profit;
            item.buyPrice = order.left == null ? null : order.left.price;
            item.sellPrice = order.right == null ? null : order.right.price;
            item.goodBuy = profit != null && profit > 3 && orderPrices.size() > 30 && trend.getDirection() == 1;
            item.avg48h = avg48h;
            item.avg90d = avg90d;
            item.orderCount = orderPrices.size();
            MarketAPI.Pair<Integer, Integer> itemCount = WFInfo.getItemCount(name);
            item.owned = itemCount.left;
            item.itemsInSet = itemCount.right;
            if (item.tracked && item.sellPrice != null && item.sellPrice <= item.trackValue) Toolkit.getDefaultToolkit().beep();

            Item finalItem = item;
            SwingUtilities.invokeLater(() -> {
                if (finalItem.location >= app.getWindow().getTableModel().getDataVector().size()) {
                    System.out.println(finalItem.name + " indexed " + finalItem.location + " in array of length " +
                            app.getWindow().getTableModel().getDataVector().size());
                    app.getWindow().getTableModel().getDataVector().add(finalItem);
                    app.getWindow()
                            .getTableModel()
                            .fireTableRowsInserted(
                                    app.getWindow().getTableModel().getDataVector().size() - 1,
                                    app.getWindow().getTableModel().getDataVector().size() - 1);
                } else {
                    app.getWindow().getTableModel().getDataVector().set(finalItem.location, finalItem);
                }
                app.getWindow().getTableModel().fireTableRowsUpdated(finalItem.location, finalItem.location);
            });

        } catch (IOException | InterruptedException e) {
            e.printStackTrace();
        }

        if (repeat) pushTask(item);
    }

    public void purgeQueue() {
        itemQueue.clear();
    }

    public boolean isPaused() {
        return this.paused;
    }

    public void setPaused(boolean paused) {
        this.paused = paused;
    }

    public void terminate() {
        shouldRun = false;
    }
}
